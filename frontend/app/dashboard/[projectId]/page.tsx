...
function ProjectLiquidNav({ active, onChange }: { active: ProjTab; onChange: (t: ProjTab) => void }) {
  ...
  const dragIdx   = useRef<number>(PROJECT_TABS.findIndex(t => t.id === active));
  const getIdx = useCallback(() => PROJECT_TABS.findIndex(t => t.id === active), [active]);

  useEffect(() => {
    const pill = pillRef.current; const bar = barRef.current;
    if (!pill || !bar) return;
    const tw = bar.offsetWidth / 4;
    pill.style.left  = `${tw * getIdx() + 4}px`;
    pill.style.width = `${tw - 8}px`;
  }, [active, getIdx]);

  const onDown = (e: React.PointerEvent) => { dragStart.current = e.clientX; dragIdx.current = getIdx(); (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId); };
  const onMove = (e: React.PointerEvent) => {
    if (dragStart.current === null) return;
    const bar = barRef.current; if (!bar) return;
    const tw = bar.offsetWidth / 4;
    const moved = Math.round((e.clientX - dragStart.current) / tw);
    const ni = Math.max(0, Math.min(3, dragIdx.current + moved));
    if (PROJECT_TABS[ni].id !== active) onChange(PROJECT_TABS[ni].id);
  };
  const onUp = () => { dragStart.current = null; };

  return (
    <div className="liquid-glass-bar relative flex" style={{ padding: "4px", height: 60 }}
      ref={barRef} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp}>
      <div className="liquid-glass-pill" ref={pillRef} />
      {PROJECT_TABS.map(({ id, label, Icon }) => {
        const act = id === active;
        return (
          <div key={id} className="lgnav-tab flex-1" onClick={() => onChange(id)}>
            <Icon size={16} style={{ color: act?"#fff":"rgba(255,255,255,0.4)", filter: act?"drop-shadow(0 0 5px rgba(255,255,255,0.5))":"none", transition:"all 0.3s" }} />
            <span style={{ fontSize:9,fontWeight:600,color:act?"#fff":"rgba(255,255,255,0.35)",transition:"all 0.3s",fontFamily:"var(--font-sans)",letterSpacing:"0.03em" }}>
              {label}
            </span>
          </div>
        );
